 ...
 * 微信公众号（游走在数据之间）
 * @author AgileQ
 * @createDate 2019年7月23日 下午5:01:12
 * @version v0.1
 * @describe KafkaConsumerDialog
 */
public class KafkaConsumerDialog extends BaseStepDialog implements
	...
	private TextVar wPartitionName;
	private TextVar wAssignPartition;
	private TextVar wOffsetName;
	private TextVar wOffsetVal;
	...
	public KafkaConsumerDialog(Shell parent, Object in, TransMeta tr,
			String sname) {
		super(parent, (BaseStepMeta) in, tr, sname);
		consumerMeta = (KafkaConsumerMeta) in;
	}

	public KafkaConsumerDialog(Shell parent, BaseStepMeta baseStepMeta,
			TransMeta transMeta, String stepname) {
		super(parent, baseStepMeta, transMeta, stepname);
		consumerMeta = (KafkaConsumerMeta) baseStepMeta;
	}

	public KafkaConsumerDialog(Shell parent, int nr, BaseStepMeta in,
			TransMeta tr) {
	...
	public String open() {
		...
		wlStepname.setText(Messages
				.getString("KafkaConsumerDialog.StepName.Label"));
		...
		wlTopicName.setText(Messages
				.getString("KafkaConsumerDialog.TopicName.Label"));
		...
		wlFieldName.setText(Messages
				.getString("KafkaConsumerDialog.FieldName.Label"));
		...
		wlKeyFieldName.setText(Messages
				.getString("KafkaConsumerDialog.KeyFieldName.Label"));
		...
		wKeyFieldName = new TextVar(transMeta, shell, SWT.SINGLE | SWT.LEFT
				| SWT.BORDER);
		...
		// Partition name
		Label wlPartitionName = new Label(shell, SWT.RIGHT);
		wlPartitionName.setText("Partition name");
		props.setLook(wlPartitionName);
		FormData fdlPartitionName = new FormData();
		fdlPartitionName.top = new FormAttachment(lastControl, margin);
		fdlPartitionName.left = new FormAttachment(0, 0);
		fdlPartitionName.right = new FormAttachment(middle, -margin);
		wlPartitionName.setLayoutData(fdlPartitionName);
		wPartitionName = new TextVar(transMeta, shell, SWT.SINGLE | SWT.LEFT
				| SWT.BORDER);
		props.setLook(wPartitionName);
		wPartitionName.addModifyListener(lsMod);
		FormData fdPartitionName = new FormData();
		fdPartitionName.top = new FormAttachment(lastControl, margin);
		fdPartitionName.left = new FormAttachment(middle, 0);
		fdPartitionName.right = new FormAttachment(100, 0);
		wPartitionName.setLayoutData(fdPartitionName);
		lastControl = wPartitionName;

		// Assign Partition
		Label wlAssignPartition = new Label(shell, SWT.RIGHT);
		wlAssignPartition.setText("Assign Partition");
		props.setLook(wlAssignPartition);
		FormData fdlAssignPartition = new FormData();
		fdlAssignPartition.top = new FormAttachment(lastControl, margin);
		fdlAssignPartition.left = new FormAttachment(0, 0);
		fdlAssignPartition.right = new FormAttachment(middle, -margin);
		wlAssignPartition.setLayoutData(fdlAssignPartition);
		wAssignPartition = new TextVar(transMeta, shell, SWT.SINGLE | SWT.LEFT
				| SWT.BORDER);
		props.setLook(wAssignPartition);
		wAssignPartition.addModifyListener(lsMod);
		FormData fdAssignPartition = new FormData();
		fdAssignPartition.top = new FormAttachment(lastControl, margin);
		fdAssignPartition.left = new FormAttachment(middle, 0);
		fdAssignPartition.right = new FormAttachment(100, 0);
		wAssignPartition.setLayoutData(fdAssignPartition);
		lastControl = wAssignPartition;

		// Offset name
		Label wlOffsetName = new Label(shell, SWT.RIGHT);
		wlOffsetName.setText("Offset name");
		props.setLook(wlOffsetName);
		FormData fdlOffsetName = new FormData();
		fdlOffsetName.top = new FormAttachment(lastControl, margin);
		fdlOffsetName.left = new FormAttachment(0, 0);
		fdlOffsetName.right = new FormAttachment(middle, -margin);
		wlOffsetName.setLayoutData(fdlOffsetName);
		wOffsetName = new TextVar(transMeta, shell, SWT.SINGLE | SWT.LEFT
				| SWT.BORDER);
		props.setLook(wOffsetName);
		wOffsetName.addModifyListener(lsMod);
		FormData fdOffsetName = new FormData();
		fdOffsetName.top = new FormAttachment(lastControl, margin);
		fdOffsetName.left = new FormAttachment(middle, 0);
		fdOffsetName.right = new FormAttachment(100, 0);
		wOffsetName.setLayoutData(fdOffsetName);
		lastControl = wOffsetName;

		// Offset value
		Label wlOffsetVal = new Label(shell, SWT.RIGHT);
		wlOffsetVal.setText("Offset value");
		props.setLook(wlOffsetVal);
		FormData fdlOffsetVal = new FormData();
		fdlOffsetVal.top = new FormAttachment(lastControl, margin);
		fdlOffsetVal.left = new FormAttachment(0, 0);
		fdlOffsetVal.right = new FormAttachment(middle, -margin);
		wlOffsetVal.setLayoutData(fdlOffsetVal);
		wOffsetVal = new TextVar(transMeta, shell, SWT.SINGLE | SWT.LEFT
				| SWT.BORDER);
		props.setLook(wOffsetVal);
		wOffsetVal.addModifyListener(lsMod);
		FormData fdOffsetVal = new FormData();
		fdOffsetVal.top = new FormAttachment(lastControl, margin);
		fdOffsetVal.left = new FormAttachment(middle, 0);
		fdOffsetVal.right = new FormAttachment(100, 0);
		wOffsetVal.setLayoutData(fdOffsetVal);
		lastControl = wOffsetVal;

		// Group ID
		Label wlGroupId = new Label(shell, SWT.RIGHT);
		wlGroupId.setText("group.id");
		...
		wGroupId = new TextVar(transMeta, shell, SWT.SINGLE | SWT.LEFT
				| SWT.BORDER);
		props.setLook(wGroupId);
		wGroupId.addModifyListener(lsMod);
		FormData fdGroupId = new FormData();
		fdGroupId.top = new FormAttachment(lastControl, margin);
		...
		wLimit = new TextVar(transMeta, shell, SWT.SINGLE | SWT.LEFT
				| SWT.BORDER);
		...
		wlTimeout.setText(Messages
				.getString("KafkaConsumerDialog.Timeout.Label"));
		...
		wlStopOnEmptyTopic.setText(Messages
				.getString("KafkaConsumerDialog.StopOnEmpty.Label"));
		...
		ColumnInfo[] colinf = new ColumnInfo[] {
				new ColumnInfo(
						Messages.getString("KafkaConsumerDialog.TableView.NameCol.Label"),
						ColumnInfo.COLUMN_TYPE_TEXT, false),
				new ColumnInfo(
						Messages.getString("KafkaConsumerDialog.TableView.ValueCol.Label"),
						ColumnInfo.COLUMN_TYPE_TEXT, false), };

		wProps = new TableView(transMeta, shell,
		...
		wPartitionName.addSelectionListener(lsDef);
		wAssignPartition.addSelectionListener(lsDef);
		wOffsetName.addSelectionListener(lsDef);
		wOffsetVal.addSelectionListener(lsDef);
		wGroupId.addSelectionListener(lsDef);
	...
	private void getData(KafkaConsumerMeta consumerMeta, boolean copyStepname) {
		...
		wPartitionName.setText(Const.NVL(consumerMeta.getPartition(), ""));
		wAssignPartition.setText(Const.NVL(consumerMeta.getAssignPartition(),
				""));
		wOffsetName.setText(Const.NVL(consumerMeta.getOffset(), ""));
		wOffsetVal.setText(Const.NVL(consumerMeta.getOffsetVal(), ""));
		wGroupId.setText(Const.NVL(consumerMeta.getGroupId(), ""));
		...
		propNames.addAll(Arrays
				.asList(KafkaConsumerMeta.KAFKA_PROPERTIES_NAMES));
		propNames.addAll(consumerMeta.getKafkaProperties()
				.stringPropertyNames());
		...
		for (String propName : propNames) {
			...
			TableItem item = new TableItem(wProps.table, i++ > 1 ? SWT.BOLD
					: SWT.NONE);
			int colnr = 1;
			item.setText(colnr++, Const.NVL(propName, ""));
			String defaultValue = KafkaConsumerMeta.KAFKA_PROPERTIES_DEFAULTS
					.get(propName);
	...
	private void setData(KafkaConsumerMeta consumerMeta) {
		...
		consumerMeta.setPartition(wPartitionName.getText());
		consumerMeta.setAssignPartition(wAssignPartition.getText());
		consumerMeta.setOffset(wOffsetName.getText());
		consumerMeta.setOffsetVal(wOffsetVal.getText());
		consumerMeta.setGroupId(wGroupId.getText());
