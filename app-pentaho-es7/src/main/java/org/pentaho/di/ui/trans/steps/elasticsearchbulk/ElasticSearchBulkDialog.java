...
public class ElasticSearchBulkDialog extends BaseStepDialog implements
  ...
  */
	...
	private void fillIndexGroup(Composite parentTab) {
		...
		new LabelTextVar(transMeta, wIndexGroup, BaseMessages.getString(PKG,
				"ElasticSearchBulkDialog.Type"
	...
	private void addServersTab() {
		...
		new TableView(transMeta, wServersComp, SWT.BORDER | SWT.FULL_SELECTION
				| SWT.MULTI, columnsMeta, 1, lsMod,
	...
	private void addSettingsTab() {
		...
		new TableView(transMeta, wSettingsComp, SWT.BORDER | SWT.FULL_SELECTION
				| SWT.MULTI, columnsMeta, 1, lsMod,
	...
	private void addFieldsTab() {
		...
		new TableView(transMeta, wFieldsComp, SWT.BORDER | SWT.FULL_SELECTION
				| SWT.MULTI, columnsMeta,
	...
	private void fillOptionsGroup(Composite parentTab) {
		...
		new LabelTimeComposite(wSettingsGroup, BaseMessages.getString(PKG,
				"ElasticSearchBulkDialog.TimeOut"
		...
		"ElasticSearchBulkDialog.IdOutField.Label"), BaseMessages.getString(
				PKG,
		...
		Control[] settingsControls = new Control[] { wlBatchSize, wBatchSize,
				wlStopOnError, wStopOnError, wTimeOut,

				wIdInField, wlIsOverwrite, wIsOverwrite, wlUseOutput,
				wUseOutput, wIdOutField, wlIsJson, wIsJson,
	...
	private void getPreviousFields(TableView table) {
		...
		try {
			...
			new ErrorDialog(shell, BaseMessages.getString(PKG,
					"System.Dialog.GetFieldsFailed.Title"), BaseMessages
	...
	private void test(TestType testType) {
		...
		try {
			...
			if (!tempMeta.getServers().isEmpty()) {
				...
				// try ( PreBuiltTransportClient client = new
				...
				// InetAddress.getByName( transMeta.environmentSubstitute(
				// server.getAddress() ) ),
				...
				for (Server server : tempMeta.getServers()) {
					...
					rclient = new RestHighLevelClient(
							RestClient.builder(new HttpHost(
									server.getAddress(), Integer.valueOf(server
											.getPort()), "http")));
				...
				boolean exists = rclient.indices().exists(request,
				...
				case INDEX:
					...
					// IndicesExistsRequestBuilder indicesExistBld =
					// admin.indices().prepareExists( tempMeta.getIndex() );

					// IndicesExistsResponse indicesExistResponse =
					// indicesExistBld.execute().get();
					...
					// RecoveryRequestBuilder indicesBld =
					// rclient.indices().prepareRecoveries( tempMeta.getIndex()
					// );

					// ActionFuture<RecoveryResponse> lafInd =
					// indicesBld.execute();
					...
					showMessage(BaseMessages.getString(PKG,
							"ElasticSearchBulkDialog.TestIndex.TestOK", "true"));

					break;

				case CLUSTER:

					// ClusterStateRequestBuilder clusterBld =
					// admin.cluster().prepareState();

					// ActionFuture<ClusterStateResponse> lafClu =
					// clusterBld.execute();
					...
					MainResponse response = rclient
							.info(RequestOptions.DEFAULT);

					showMessage(BaseMessages.getString(PKG,
							"ElasticSearchBulkDialog.TestCluster.TestOK",
							response.getClusterName(), response.getVersion()));
