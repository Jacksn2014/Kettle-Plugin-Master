...
public class KafkaConsumerMeta extends BaseStepMeta implements
	...
	public static final String[] KAFKA_PROPERTIES_NAMES = new String[] {
			"zookeeper.connect", "group.id", "consumer.id",
			"socket.timeout.ms", "socket.receive.buffer.bytes",
			"fetch.message.max.bytes", "auto.commit.interval.ms",
			"queued.max.message.chunks", "rebalance.max.retries",
			"fetch.min.bytes", "fetch.wait.max.ms", "rebalance.backoff.ms",
			"refresh.leader.backoff.ms", "auto.commit.enable",
			"auto.offset.reset", "consumer.timeout.ms", "client.id",
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (keyField == null) {
			remarks.add(new CheckResult(
					...
					Messages.getString("KafkaConsumerMeta.Check.InvalidKeyField"),
					stepMeta));
		}
		try {
	...
	public String getXML() throws KettleException {
		...
		if (partition != null) {
			retval.append("    ").append(
					XMLHandler.addTagValue("PARTITION", partition));
		}
		if (offset != null) {
			retval.append("    ").append(
					XMLHandler.addTagValue("OFFSET", offset));
		}
		if (offsetVal != null) {
			retval.append("    ").append(
					XMLHandler.addTagValue("OFFSETVAL", offsetVal));
		}
		if (limit != null) {
			retval.append("    ")
		...
		if (stopOnEmptyTopic) {
			retval.append("    ").append(
					XMLHandler.addTagValue("STOPONEMPTYTOPIC", "true"));
		}
	...
	public void readRep(Repository rep, ObjectId stepId,
		...
		try {
			...
			if (kafkaPropsXML != null) {
				kafkaProperties.loadFromXML(new ByteArrayInputStream(
						kafkaPropsXML.getBytes()));
			}
