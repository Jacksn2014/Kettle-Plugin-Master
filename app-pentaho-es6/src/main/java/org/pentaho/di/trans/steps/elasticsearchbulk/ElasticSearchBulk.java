 ...
 * @createDate 2019年7月24日 下午2:16:05
 * @version v0.1
...
public class ElasticSearchBulk extends BaseStep implements StepInterface {
	 ...
	 */
	private void addSourceFromJsonString(Object[] row,
			IndexRequestBuilder requestBuilder) throws KettleStepException {
	...
	private void addSourceFromRowFields(IndexRequestBuilder requestBuilder,
			RowMetaInterface rowMeta, Object[] row) throws IOException {
		...
		for (int i = 0; i < rowMeta.size(); i++) {
			...
			// 对于null值做特殊处理，以规避XContentBuilder校验
	...
	private void initFromMeta() {
		...
		this.opType = StringUtils.isNotBlank(meta.getIdInField())
	...
	private void addIdToRow(String id, int rowIndex) {
		...
		data.inputRowBuffer[rowIndex] = RowDataUtil.resizeArray(
				data.inputRowBuffer[rowIndex], getInputRowMeta().size() + 1);
